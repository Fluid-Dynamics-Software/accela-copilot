...
//if addNum is null, condition is added to all addresses on CAP
{
	if (!addNum) {
		...
		if (capAddResult.getSuccess()) {
			...
			for (zz in Adds) {
				...
				if (Adds[zz].getRefAddressId()) {
					var addAddCondResult = aa.addressCondition.addAddressCondition(Adds[zz].getRefAddressId(), cType, cDesc, cComment, null, null, cImpact, cStatus, sysDate, null,
							sysDate, sysDate, systemUserObj, systemUserObj);
		...
		var addAddCondResult = aa.addressCondition.addAddressCondition(addNum, cType, cDesc, cComment, null, null, cImpact, cStatus, sysDate, null, sysDate, sysDate,
				systemUserObj, systemUserObj);
				...
				if (!addNum) // add to all reference address on the current capId
				{
					...
					if (capAddResult.getSuccess()) {
						...
						for (zz in Adds) {
							...
							if (Adds[zz].getRefAddressId()) {
								var addAddCondResult = aa.addressCondition.addAddressCondition(Adds[zz].getRefAddressId(), standardCondition.getConditionType(), standardCondition
										.getConditionDesc(), standardCondition.getConditionComment(), null, null, standardCondition.getImpactCode(), cStatus, sysDate, null,
										sysDate, null, systemUserObj, systemUserObj)
					...
					var addAddCondResult = aa.addressCondition.addAddressCondition(addNum, standardCondition.getConditionType(), standardCondition.getConditionDesc(),
							standardCondition.getConditionComment(), null, null, standardCondition.getImpactCode(), cStatus, sysDate, null, sysDate, null, systemUserObj,
							systemUserObj)
...
function addAppCondition(cType, cStatus, cDesc, cComment, cImpact, conditionOfApproval) {
	...
	if (arguments.length > 5) {
		addCapCondResult = aa.capCondition.addCapCondition(capId, cType, cDesc, cComment, sysDate, null, sysDate, null, null, cImpact, systemUserObj, systemUserObj, cStatus,
				currentUserID, "A", conditionOfApproval);
	} else {
		addCapCondResult = aa.capCondition.addCapCondition(capId, cType, cDesc, cComment, sysDate, null, sysDate, null, null, cImpact, systemUserObj, systemUserObj, cStatus,
				currentUserID, "A");
	}
...
function addFeeByDate(feeCap, fdate, fcode, fsched, fperiod, fqty, finvoice) // Adds a single fee, optional argument: fCap
{
	...
	if (assessFeeResult.getSuccess()) {
		feeSeq = assessFeeResult.getOutput();
		logDebug("Successfully added Fee:" + fcode + ", Qty:" + fqty + " Version:" + v + " Sched: " + fsched + " based on date " + vDate + " " + feeCapMessage
				+ ".  The assessed fee Sequence Number " + feeSeq + feeCapMessage);
...
function addLicenseCondition(cType, cStatus, cDesc, cComment, cImpact) {
	...
	for ( var refLic in refLicArr) {
		...
		if (licSeq >= 0) {
			var addCAEResult = aa.caeCondition.addCAECondition(licSeq, cType, cDesc, cComment, null, null, cImpact, cStatus, sysDate, null, sysDate, sysDate, systemUserObj,
					systemUserObj)
				...
				if (!licSeqNum) // add to all reference licenses on the current capId
				{
					...
					for ( var refLic in refLicArr)
						if (refLicArr[refLic].getLicenseProfessionalModel().getLicSeqNbr()) {
							licSeq = refLicArr[refLic].getLicenseProfessionalModel().getLicSeqNbr();
							var addCAEResult = aa.caeCondition.addCAECondition(licSeq, standardCondition.getConditionType(), standardCondition.getConditionDesc(),
									standardCondition.getConditionComment(), null, null, standardCondition.getImpactCode(), cStatus, sysDate, null, sysDate, sysDate,
									systemUserObj, systemUserObj);
					...
					var addCAEResult = aa.caeCondition.addCAECondition(licSeqNum, standardCondition.getConditionType(), standardCondition.getConditionDesc(), standardCondition
							.getConditionComment(), null, null, standardCondition.getImpactCode(), cStatus, sysDate, null, sysDate, sysDate, systemUserObj, systemUserObj);
	...
	if (!parcelNum) {
		...
		if (capParcelResult.getSuccess()) {
			...
			for (zz in Parcels) {
				...
				var addParcelCondResult = aa.parcelCondition.addParcelCondition(Parcels[zz].getParcelNumber(), cType, cDesc, cComment, null, null, cImpact, cStatus, sysDate, null,
						sysDate, sysDate, systemUserObj, systemUserObj);
		...
		var addParcelCondResult = aa.parcelCondition.addParcelCondition(parcelNum, cType, cDesc, cComment, null, null, cImpact, cStatus, sysDate, null, sysDate, sysDate,
				systemUserObj, systemUserObj);
...
function addrAddCondition(pAddrNum, pType, pStatus, pDesc, pComment, pImpact, pAllowDup) {
	...
	if (!pAddrNum) //no address num, add condition to all addresses on CAP
	{
		...
		if (capAddrResult.getSuccess()) {
			...
			for (zz in addresses) {
				...
				if (noDup) //Check if this address has duplicate condition
				{
					...
					if (condArray.length > 0) {
						for (bb in condArray) {
							...
							if (cImpact == null)
								cImpact = " ";
							if ((pType == null || pType.toUpperCase() == cType.toUpperCase()) && (pStatus == null || pStatus.toUpperCase() == cStatus.toUpperCase())
									&& (pDesc == null || pDesc.toUpperCase() == cDesc.toUpperCase()) && (pImpact == null || pImpact.toUpperCase() == cImpact.toUpperCase())) {
				...
				addCondResult = aa.addressCondition.addAddressCondition(addrRefId, pType, pDesc, pComment, null, null, pImpact, pStatus, sysDate, null, sysDate, sysDate,
						systemUserObj, systemUserObj);
	...
	} else //add condition to specified address only
	{
		...
		var addCondResult = aa.addressCondition.addAddressCondition(pAddrNum, pType, pDesc, pComment, null, null, pImpact, pStatus, sysDate, null, sysDate, sysDate, systemUserObj,
				systemUserObj);
		if (addCondResult.getSuccess()) {
	...
	if (!aa.capCondition.getStandardConditions) {
		...
		//if (standardConditions[i].getConditionType().toUpperCase() == cType.toUpperCase() && standardConditions[i].getConditionDesc().toUpperCase() == cDesc.toUpperCase()) //EMSE Dom function does like search, needed for exact match
		{
			...
			var addCapCondResult = aa.capCondition.addCapCondition(itemCap, standardCondition.getConditionType(), standardCondition.getConditionDesc(), standardCondition
					...
					standardCondition.getResolutionAction(), null, null, standardCondition.getConditionNbr(), standardCondition.getConditionGroup(), standardCondition
							.getDisplayNoticeOnACA(), standardCondition.getDisplayNoticeOnACAFee(), standardCondition.getPriority(), standardCondition.getConditionOfApproval());
	...
	if (!iInspector) // must be a department name?
	{
		...
		for ( var thisdpt in dpt) {
			...
			if (iName.equals(m.getDeptName())) {
				iNameResult = aa.person.getUser(null, null, null, null, m.getAgencyCode(), m.getBureauCode(), m.getDivisionCode(), m.getSectionCode(), m.getGroupCode(), m
						.getOfficeCode());
...
function closeSubWorkflow(thisProcessID, wfStat) // optional capId
{
	...
	var handleResult = aa.workflow.handleDisposition(itemCap, relRecord.getStepNumber(), relRecord.getParentProcessID(), wfStat, sysDate, "Closed via script", "Closed via script",
			systemUserObj, "Y");
...
function contactObj(ccsm) {
	...
	this.getContactName = function(includeContactName, includeBusinessName, includeTradeName) {
		...
		if (!matches(includeContactName, null, undefined, "")) {
			vIncludeContactName = true
		}
		if (!matches(includeBusinessName, null, undefined, "")) {
			vIncludeBusinessName = true
		}
		if (!matches(includeTradeName, null, undefined, "")) {
			vIncludeTradeName = true
		}
	...
	this.createRefLicProf = function(licNum, rlpType, addressType, licenseState) {
		...
		if (peop.getMaskedSsn() != null)
			newLic.setMaskedSsn(peop.getMaskedSsn());
		if (peop.getFein() != null)
			newLic.setFein(peop.getFein());
		if (peop.getCountry() != null)
			newLic.setCountry(peop.getCountry());
		if (peop.getCountryCode() != null)
			newLic.setCountryCode(peop.getCountryCode());
		...
		if (addr.getCountryCode() != null)
			newLic.setCountryCode(addr.getCountryCode());
				...
				if (addrType == addressType) {
					...
					if (cResult.getSuccess()) {
						...
						if (vOverwrite) {
							...
							if (effectiveDate)
								casm.setEffectiveDate(aa.util.parseDate(effectiveDate));
							if (expirationDate)
								casm.setExpirationDate(aa.util.parseDate(expirationDate));
...
function copyConditions(fromCapId) // optional toCapID
{
	...
	for (cc in condA) {
		...
		var addCapCondResult = aa.capCondition.addCapCondition(itemCap, thisC.getConditionType(), thisC.getConditionDescription(), thisC.getConditionComment(), thisC
				.getEffectDate(), thisC.getExpireDate(), sysDate, thisC.getRefNumber1(), thisC.getRefNumber2(), thisC.getImpactCode(), thisC.getIssuedByUser(), thisC
				.getStatusByUser(), thisC.getConditionStatus(), currentUserID, String("A"), null, thisC.getDisplayConditionNotice(), thisC.getIncludeInConditionName(), thisC
				.getIncludeInShortDescription(), thisC.getInheritable(), thisC.getLongDescripton(), thisC.getPublicDisplayMessage(), thisC.getResolutionAction(), null, null, thisC
				.getReferenceConditionNumber(), thisC.getConditionGroup(), thisC.getDisplayNoticeOnACA(), thisC.getDisplayNoticeOnACAFee(), thisC.getPriority(), thisC
				.getConditionOfApproval());
		if (addCapCondResult.getSuccess())
...
function copyConditionsFromParcel(parcelIdString) {
	...
	for (cc in condA) {
		...
		if (!appHasCondition(thisC.getConditionType(), null, thisC.getConditionDescription(), thisC.getImpactCode())) {
			var addCapCondResult = aa.capCondition.addCapCondition(capId, thisC.getConditionType(), thisC.getConditionDescription(), thisC.getConditionComment(), thisC
					.getEffectDate(), thisC.getExpireDate(), sysDate, thisC.getRefNumber1(), thisC.getRefNumber2(), thisC.getImpactCode(), thisC.getIssuedByUser(), thisC
					.getStatusByUser(), thisC.getConditionStatus(), currentUserID, "A", thisC.getConditionStatusType(), thisC.getDisplayConditionNotice(), thisC
					.getIncludeInConditionName(), thisC.getIncludeInShortDescription(), thisC.getInheritable(), thisC.getLongDescripton(), thisC.getPublicDisplayMessage(), thisC
					.getResolutionAction(), null, null, thisC.getConditionNumber(), thisC.getConditionGroup(), thisC.getDisplayNoticeOnACA(), thisC.getDisplayNoticeOnACAFee());
			if (addCapCondResult.getSuccess())
				logDebug("Successfully added condition (" + thisC.getImpactCode() + ") " + thisC.getConditionDescription());
			else
		...
		if (!aa.calendar.getNextWorkDay) {
			...
			while (i < Math.abs(amt)) {
				if (amt > 0) {
					...
					dDate = new Date(aa.calendar.getPreviousWorkDay(aa.date.parseDate(dDate.getMonth() + 1 + "/" + dDate.getDate() + "/" + dDate.getFullYear())).getOutput()
							.getTime());
					i++;
...
function getCapId() {
	...
	if (s_id1 == null || s_id1 == "" || s_id2 == null || s_id2 == "" || s_id3 == null || s_id3 == "") {
		return null;
	}
	...
	for ( var thisAddr in addrArray)
		if (addrArray[thisAddr].getRefAddressId()) {
			...
			for ( var thisAddrCond in addrCondArray) {
				...
				if ((pType == null || pType.toUpperCase().equals(cType.toUpperCase())) && (pStatus == null || pStatus.toUpperCase().equals(cStatus.toUpperCase()))
						&& (pDesc == null || pDesc.toUpperCase().equals(cDesc.toUpperCase())) && (pImpact == null || pImpact.toUpperCase().equals(cImpact.toUpperCase()))) {
					var r = new condMatchObj;
					r.objType = "Address";
	...
	if (inspResultObj.getSuccess()) {
		...
		for (xx in inspList)
			if (String(insp2Check).equals(inspList[xx].getInspectionType())) {
				...
				inspUserObj = aa.person.getUser(inspList[xx].getInspector().getFirstName(), inspList[xx].getInspector().getMiddleName(), inspList[xx].getInspector().getLastName())
						.getOutput();
...
function licenseProfObject(licnumber, lictype, vCapId) {
	...
	this.getEmailTemplateParams = function(params, vLicenseType, reference) {
		...
		if (vLicenseType)
			vLicType = vLicenseType;
		if (reference == false)
			vFromReferenece = false;
	...
	this.getMaxRowByTable = function(vTableName) {
		...
		if (this.validTables) {
			...
			if (tbl != null) {
				for (row in tbl)
					for (col in tbl[row])
						//due to way data is stored must loop through all row/columns
		...
		if (this.valid) {
			...
			if (capLicenseArr != null) {
				for (capLic in capLicenseArr) {
					...
					if (lpsm.getLicenseNbr() + "" == this.refLicModel.getStateLicense() + "" && lpsm.getLicenseType() + "" == this.refLicModel.getLicenseType() + "") {
	...
	this.updateFromRecordContactByType = function(vCapId, vContactType, vUpdateAddress, vUpdatePhoneEmail, vAddressType) {
		...
		if (this.valid) {
			...
			for (contact in conArr) {
				if (vContactType.toString().toUpperCase() == conArr[contact].getPeople().getContactType().toString().toUpperCase()
...
function loadASITables4ACA() {
	...
	for (var ta = gm.getTablesMap(), tai = ta.values().iterator(); tai.hasNext();) {
		var tsm = tai.next();
		if (!tsm.rowIndex.isEmpty()) {
			var tempObject = new Array, tempArray = new Array, tn = tsm.getTableName();
			tn = String(tn).replace(/[^a-zA-Z0-9]+/g, ""), isNaN(tn.substring(0, 1)) || (tn = "TBL" + tn);
			...
			tempArray.push(tempObject);
			var copyStr = "" + tn + " = tempArray";
			logDebug("ASI Table Array : " + tn + " (" + numrows + " Rows)"), eval(copyStr)
		}
	}
}
...
function pairObj(actID) {
	...
	this.load = function(loadStr) {
		...
		if (loadArr.length < 2 || loadArr.length > 3) {
			logMessage("**ERROR: The following Criteria/Action pair is incorrectly formatted.  Two or three elements separated by a caret (\"^\") are required. " + br + br
					+ loadStr)
		} else {
			this.cri = loadArr[0];
			this.act = loadArr[1];
...
function paymentByTrustAccount(fSeqNbr) //optional: itemCap
{
	...
	applyResult = aa.finance.applyPayment(itemCap, pR.getPaymentSeqNbr(), feeAmount, feeSeqNbrArray, invNbrArray, feeAllocArray, aa.date.getCurrentDate(), "Paid", "Paid", pR
			.getCashierID(), null);
...
function removeTask(targetCapId, removeTaskName) // optional process name
{
	...
	for (i in wfObj) {
		var fTask = wfObj[i];
		if (fTask.getTaskDescription().toUpperCase().equals(removeTaskName.toUpperCase())
				&& (!useProcess || fTask.getProcessCode().toUpperCase().equals(processName.toUpperCase()))) {
			tTask = wfObj[i];
		}
	}
...
function transferFeesAndPayments(sourceCapId, targetCapId) {
	...
	var xferResult = aa.finance.makeFundTransfer(capId, targetCapId, currentUserID, "", "", sysDate, sysDate, "", sysDate, unapplied, "NA", "Fund Transfer", "NA", "R", null, "",
			"NA", "");
...
// needs function paymentGetNotAppliedTot()
{
	...
	if (getCapResult.getSuccess()) {
		...
		var xferResult = aa.finance.makeFundTransfer(capId, parentId, currentUserID, "", "", sysDate, sysDate, "", sysDate, dollarAmount, "NA", "Fund Transfer", "NA", "R", null,
				"", "NA", "");
...
function transferReceiptAndApply(receiptCapId, targetCapId) {
	...
	var xferResult = aa.finance.makeFundTransfer(receiptCapId, targetCapId, currentUserID, "", "", sysDate, sysDate, "", sysDate, transferAmt, "NA", "Fund Transfer", "NA", "R",
			null, "", "NA", "");
	if (xferResult.getSuccess())
...
function updateAppStatus(stat, cmt) // optional cap id
{
	...
	if (updateStatusResult.getSuccess())
		...
		logDebug("**ERROR: application status update to " + stat + " was unsuccessful.  The reason is " + updateStatusResult.getErrorType() + ":"

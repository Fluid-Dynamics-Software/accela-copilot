...
UTAG.prototype.getOnloginSettings = function (publicUser){
	...
	while (asit.nextRow()){
		...
		if (action == "Custom Amend ..."){
			...
			scriptEMSEForLabel += " 	return 'DASHBOARD:GET_REPORT'; ";
			scriptEMSEForLabel += " } ";
			...
			// @ Collect reports and push to array
			
			if (!object.actions.reports) {
				object.actions.reports = [];
			}

			object.actions.reports.push({
				action: action,
				name: processedActionLabel,
				reportName: emseScript,
				icon: actionIcon || processedActionIcon,
				translation: { "ar-AE": actionLabelAr || "" },
				condition: scriptEMSECondition,
				script: scriptEMSEForLabel,
			});
		...
		for (action in newUtagSettings.actions){
			if (newUtagSettings.actions.hasOwnProperty(action)){
				...
				if (action == "emse"){
					...
					for (var r in newUtagSettings.actions.reports) {
						if (newUtagSettings.actions.reports.hasOwnProperty(r)) {
							var report = { script: "", condition: "", icon: "" };

							if (newUtagSettings.actions.reports[r].script) {
								report.script = " " + newUtagSettings.actions.reports[r].script;
							}
							if (newUtagSettings.actions.reports[r].reportName) {
								report.reportName = "" + newUtagSettings.actions.reports[r].reportName;
							}
							if (newUtagSettings.actions.reports[r].condition) {
								report.condition += " " + newUtagSettings.actions.reports[r].condition;
							}
							if (newUtagSettings.actions.reports[r].translation) {
								report.translation = newUtagSettings.actions.reports[r].translation;
							}
							if (newUtagSettings.actions.reports[r].icon && newUtagSettings.actions.reports[r].icon != "") {
								report.icon = newUtagSettings.actions.reports[r].icon;
							}
							if (newUtagSettings.actions.reports[r].name) {
								report.name = newUtagSettings.actions.reports[r].name;
							}
							if (!targetUtagSettings.actions.reports) {
								targetUtagSettings.actions.reports = [];
							}
							targetUtagSettings.actions.reports.push(report)
						
						}
					}
					logDebug("targetUtagSettings", JSON.stringify(targetUtagSettings));
...
function execQuery(sql) {
			...
			for (var i = 0; i < numcols; i++) {
				...
				if (result == null) {
					...
					if (result.getClass && result.getClass().getName() == "java.sql.Timestamp") {
						...
						record[columnName + "_TSTP"] = String(new Date(rs.getTimestamp(i + 1).getTime()).toString("yyyy-MM-dd'T'HH:mm:ss"));
					} else {
						record[columnName] = String(rs.getObject(i + 1));
					}
				}

				if (columnName == "B1_LICENSES") {
					var asd = [];
					if (String(result) != "null") {
						var theSplit = String(result).split("#");
						for (var s in theSplit) {
							var nextSplit = theSplit[s].split("$");
							asd.push({
								"LICENSE_NUMBER": String(nextSplit[0]),
								"LICENSE_TYPE": String(nextSplit[1]),
								"LICENSE_STATUS": String(nextSplit[2]),
								"LICENSE_FLAG": String(nextSplit[3])
							})
						}
					}
					record[columnName] = JSON.stringify(asd)
				}
			}
			return record;
		}
	...
	var t1 = Date.now();
	var td = t1 - t0;
	aa.print("time to execute query " + td)
	return data;
}
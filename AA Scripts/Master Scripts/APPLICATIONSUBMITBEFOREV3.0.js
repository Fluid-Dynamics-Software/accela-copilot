...
var ifFromRestAPI=false;
var additionalInfoWorkDescription =  aa.env.getValue("AdditionalInfoWorkDescription");
if(additionalInfoWorkDescription && additionalInfoWorkDescription.equals("From Rest API"))
{
	ifFromRestAPI = true;
}
...
if (SA) {
	...
	if(!ifFromRestAPI)
	{
		eval(getScriptText("INCLUDES_ACCELA_FUNCTIONS_ASB_ASI_ASIT",SA));
	}
	}
else {
	...
	if(!ifFromRestAPI)
	{
		eval(getScriptText("INCLUDES_ACCELA_FUNCTIONS_ASB_ASI_ASIT"));
	}
	}
...
java.lang.System.out.println("ASB:GENERAL EVENT STARTED!"); 
//if (appTypeArray[3] == "ANMC"){
if (!publicUser){
...
if (debug.indexOf("**ERROR") > 0)
	...
	if (cancel)
		...
		if (publicUser){
			...
			if (showMessage) aa.env.setValue("ScriptReturnMessage", '<img src="/portlets/images/sky_blue/menu/spacer.gif" onload=\'var disp = $(\"#displayBody\");disp.hide();disp.after(\"<font color=red size=+1 style=margin:20px;display:block><b>'+message+'</b></font><br><br>\");\'>');
